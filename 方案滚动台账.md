...
> **台账性质：** 项目方案文档的唯一整合入口与唯一存储。原方案与审查文件按“时间 × 功能滚动开发”顺序整合进本台账与《审查滚动台账》（`审查滚动台账.md`）；**原文件已于 2026-08-28 合并后删除，原文全文逐字存档于各条目内**。
>
> **组织规则：**
> 1. **最新在前**：条目按时间倒序排列，逐条标注日期；
> 2. **后进先出（LIFO）**：后续开发以文件顶部最新条目为优先执行依据，旧条目仅作历史上下文；两处冲突时以更新的条目为准；
> 3. **方案同步**：每完成一个开发阶段，在文件顶部追加新条目记录进展与状态，历史条目保持原样不改写；
> 4. **原文归档**：条目内的源文件原文逐字保留，仅追加台账条幅说明。
>
> **条目索引（新 → 旧）：**
>
> | # | 日期 | 条目 | 来源 | 状态 |
> |---|------|------|------|------|
> | ① | 2026-08-28 | 执行状态快照与方案同步点 | 台账原生 | 最新 |
> | ② | 2026-08-28 | 综合改进方案（整合四版审查结论）v1.1 | `综合方案_整合版.md` | 当前生效主方案 |
> | ③ | 2026-08-27 | 技术方案书 v0.2.1 | `技术方案.md` | 架构与设计基线 |

---

## 台账条目 [2026-08-28] ① 执行状态快照与方案同步点（台账原生，最新）

**功能滚动开发时间线：**

| 日期 | 功能里程碑 | 结果 |
|------|-----------|------|
| 2026-08-27 | 第 1/2/3 轮滚动审查：方案 v0.1 → v0.2 → v0.2.1 文本修订；代码审查 NO-GO | 见《审查滚动台账》 |
| 2026-08-27 | M0 契约 + Spike + 代码实现（16 契约、原子状态机、P0–P9 管线） | 基线 156 → 164 测试 |
| 2026-08-28 | 综合方案 v1.1 发布；**S1 最后一公里闭环**（native marks 主路径、request_id 预持久化、成本配额、指纹完整化、NEEDS_REVIEW 通道、Ollama think 修复） | 已闭合（条目② §一 1.3-A 三项清零） |
| 2026-08-28 | **S2 可机器判定的质量门禁**（QC 阈值矩阵 `config/qc`、数字复验、覆盖门禁、preview 隔离、BGM 快照、字体参数化、revision 递增、M-06 draft_meta 重写） | 188 passed |
| 2026-08-28 | **S3 发布硬化**：S3.1 gate 绑定与 verify、S3.2 故障注入矩阵、S3.3 隐私脱敏、S3.4 运维配额 + `cache gc` + 性能基准入库、S3.5 许可证决策矩阵 | **218 passed**，ruff 全绿 |
| 2026-08-28 | **分发形态决策：开源（形态 C）**，整体许可 AGPL-3.0，`LICENSE` 入库、`pyproject.toml` 随包声明 | 见 `docs/licenses.md` |
| 2026-08-28 | 顺带修复：P0 UTF-8 嗅探边界误杀、release_gate 非 UTF-8 终端编码崩溃 | 已含回归测试 |

**当前质量基线：** `uv run pytest` 218 passed（整合方案发布时 164，全程零回退）；`uv run ruff check src tests scripts` 全绿；`release_gate.py --media-smoke` 本地全绿（`Gate: blocked` 属设计内——`--live` 付费项待预算）。

**剩余待办（后续开发按后进先出顺序，自本清单顶部取任务）：**

1. **人工事项**（代码不代办）：字幕真值集标注（20–30 句）；发布候选付费 smoke 预算审批（每候选 <1 美元级，`release_gate.py --live`）；真实 Provider 基准 P50/P95 回填 `docs/perf.md`；高风险领域发布审批流程维持人工复核。
2. **S4 能力扩展**（择期，不阻塞发布）：`--jobs>1` 批量并行（前置：缓存并发窗口加固）、`--llm cloud`、解析增强、视觉一致性、超长分集、forced aligner 选型——详见条目② §四 阶段 S4。
3. **方案同步纪律**：每完成一轮开发/审查，在本台账顶部追加新快照条目，并同步更新条目② 的达成度表述（避免"宣称—事实"漂移）。

---

## 台账条目 [2026-08-28] ② 综合改进方案（整合四版审查结论）v1.1

> 来源：`综合方案_整合版.md`（编制/最后更新 2026-08-28）· 定位：**当前生效主方案**（LIFO 执行队列队首）。
> 以下内容为源文件原文全文。
...
## 台账条目 [2026-08-27] ③ 技术方案书 v0.2.1（文本级修订）

> 来源：`技术方案.md`（修订日期 2026-08-27）· 定位：架构与设计基线；其剩余缺口已由条目②收敛为 S1–S4 阶段，两者不一致时以更新的条目②为准。
> 以下内容为源文件原文全文。
