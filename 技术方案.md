...
> **版本:** v0.2(修订版)
> **日期:** 2026-08-27
> **状态:** 已吸收 gpt-5.6-sol 对 v0.1 的全部 P0/P1 评审意见,待复审
> **评审记录:** 见附录 D(评审报告全文:`review_report.md`)
> **目标读者:** 实现者(本文档 + 仓库内版本化契约/Prompt/样例共同构成开发依据)

**目标:** 构建一条"文档 → 视频"的全自动流水线:输入一份文档(PDF/Word/Markdown/TXT),无人值守输出一条带口播、配图、字幕的成片(MP4),失败关停、不产出"伪成功"。

**架构:** 10 阶段流水线(P0–P9),阶段产物**不可变、原子提交、可独立重跑**;数据契约由 **Pydantic 模型 + Draft 2020-12 JSON Schema** 双保险驱动;断点续跑是**崩溃安全 + 并发安全**的状态机,而非简单哈希比较。

**技术栈:** Python 3.11 + uv · PyMuPDF/python-docx(解析)· Ollama Qwen3:14b + 云 API(LLM,结构化输出)· edge-tts(语音,含边界时间戳)· FAL(图像,精确端点冻结)· ffmpeg(合成,参数数组+横竖屏参数化)· faster-whisper(ASR 回读兜底)· asyncio 编排。
...
| A4 | 单条视频**目标时长 1–10 分钟**(目标区间,非硬下限);超限一期**拒绝或人工拆分**,自动分集列入二期 | 见 P1-12 修订 |
| A5 | 图像默认走云端 FAL(精确端点冻结,见 5.1/风险 R13);本地 ComfyUI 为可选降级 | 成本/速度权衡 |
| A6 | 文档可能涉及未公开/敏感内容 → 提供 **privacy_mode** 三档并默认保守(见 7.4) | 见 P0-08 |
...
| FR-02 | 长文档分块理解:逐章摘要 + 事实表(数字/日期/专名),全部带 source_block_ids | 每个事实可追溯回原文位置 |
| FR-03 | 生成分镜讲稿:每场景 60–180 字,引用源 block/page,总时长受控 | 场景均有源引用;时长符合 render_timeline |
| FR-04 | 每分镜画面:原文视觉优先(图表/页裁切),无原文素材时生成式配图 | visual_source 正确分类 |
| FR-05 | 语音合成:中文口播,输出音频 + 实测时长 + **时间边界 marks** | 每个音频文件有 timing_marks |
...
| FR-10 | CLI:run/resume/batch/watch/report/doctor;退出码区分成功/可重试失败/需人工 | 中断后 resume 正确续跑且不重提远端请求 |
...
| NFR-02 | 断点续跑:进程崩溃安全(原子提交 + artifact_manifest 提交点),并发安全(Job 锁) |
...
| NFR-07 | 隐私边界:privacy_mode 三档,外发清单可审计,日志/PII 脱敏 |
| NFR-08 | Windows 优先、代码跨平台;单机运行,无自建服务依赖 |

### 2.3 输入/输出规格

**输入:** `*.pdf / *.docx / *.md / *.txt`。单文件 ≤ 200MB,且受解压总量/页数/像素/解析时间限制(防 ZIP bomb,见 P1-07)。扫描件 PDF:逐页判定,混合文本/OCR 文档按页混合处理(见 4.2)。

**输出(每个 Job):**

| 产物 | 规格 |
|------|------|
| 成片 | MP4,H.264 **High profile**,pix_fmt=yuv420p,25fps,1920×1080 或 1080×1920,AAC **48kHz** 双声道 128k,+faststart |
...
flowchart LR
    ...
    F --> G["P5 素材生产<br/>assets_manifest.json(并行)"]
    G --> H["P6 字幕生成<br/>subtitles.*"]
    G --> I["P7 视频合成<br/>render_manifest + staging"]
    H --> I
...
> 字幕(P6)先于合成(P7):合成依赖字幕文件烧录;P8 是唯一交付门禁,staging 产物只有通过硬门禁才晋升 final。
...
| P5 | 素材生产 | scene_plan.json | `assets_manifest.json` + 音频/图像 | edge-tts / FAL | **高并行**(限流) |
| P6 | 字幕生成 | assets_manifest + scene_plan | `subtitles.json/.srt/.ass` | timing marks / forced align / ASR 兜底 | 按场景并行 |
| P7 | 视频合成 | scene_plan + assets + subtitles | `render_manifest.json` + `staging/` | ffmpeg | — |
| P8 | 质检与发布 | staging + 全链产物 | `qc_report.json` → `final/output.mp4` | ffprobe / blackdetect / decode-to-null | — |
| P9 | 剪映草稿导出(可选) | scene_plan + assets + subtitles | `drafts/<job_id>/` | pyJianYingDraft | — |

### 3.3 设计原则

1. **数据契约驱动:** 阶段间只通过 JSON 文件通信;**Pydantic 模型为单一契约来源,自动生成 Draft 2020-12 JSON Schema** 提交入库,CI 检查无漂移。
...
6. **本地云混合(成本+隐私):** LLM 默认本地 Qwen3;whisper 本地;TTS edge-tts;图像 FAL。隐私模式决定云调用是否允许(见 7.4)。
...
report.pdf
  ...
  → P2 grounded_summary.json{逐章摘要, facts[{数字/日期/专名, source_block_ids}], coverage}
  → P3 script.json{scenes[{narration, source_block_ids, source_pages}]}
  → P4 scene_plan.json{scenes[{visual_source, image_prompt, …}]}
  → P5 assets_manifest.json{audio{path,duration_s,timing_marks}, images{path,request_id}, checkpoints}
  → P6 subtitles.json/.srt/.ass(cues 与 render_timeline 对齐)
  → P7 render_manifest.json + staging/output.mp4
  → P8 qc_report.json → final/output.mp4(原子晋升)
  → P9 drafts/<job_id>/(可选)
```
...
- 稳定性:等待 size/mtime 稳定且可打开(batch/watch 场景生产者可能仍在写);校验通过后复制到 `input/` 并计算 SHA-256;
- 资源限制(quarantine):DOCX 解压总量/条目数、图片像素、PDF 页数/对象数、解析超时;超限拒绝;
- 产出 `manifest.json`:来源路径、哈希、文件类型、privacy_mode、配置指纹、创建时间;后续所有阶段只读 `input/` 快照。
...
| 格式 | 方案 |
|------|------|
| PDF(文本型) | TOC 优先;block/span 坐标 + 字号/加粗判定层级;**注意 PyMuPDF 文本顺序≠阅读顺序,多栏文档须按坐标重排**;页眉页脚去重;表格区域识别 |
| PDF(扫描型) | **逐页判定**:某页无文本层才对该页 OCR(rapidocr-onnxruntime 本地),记录 ocr_confidence;混合文本/OCR 文档按页混合,不做整文件二分 |
| DOCX | 样式名 + outline level 映射;自定义标题样式、编号列表、文本框、图表/SmartArt(不支持项显式报告) |
| MD/TXT | 原生结构(markdown-it-py) |

**测试矩阵(必须全绿):** 文本 PDF / 多栏 PDF / 混合 OCR PDF / 扫描 PDF / 加密 PDF(拒绝)/ DOCX(含自定义样式)/ MD / TXT / 空文件 / 畸形文件 / 超限文件。

**接口:** `parse(job, cfg) -> ParsedDocument`;内嵌图片落入 `extracted_assets/` 并在 blocks 中引用。

### 4.3 P2 内容理解 `p2_understand`

**职责:** 生成**可追溯原文**的摘要与事实表——这是讲稿保真的基础(P0-02 修订核心)。

```json
{
  "schema_version": "1.0",
  "doc_summary": "150–300 字总览",
  "key_points": ["要点(附 source_block_ids)"],
  ...
  "facts": [
    { "kind": "number|date|proper_noun|unit|negation",
      "text": "2025 年营收增长 23%", "source_block_ids": ["b42"], "source_pages": [12] }
  ],
...
1. 用**目标模型 tokenizer 实测计数**切块(不能用"12000 字符 ≈ 6000 token"估算);每块 ≤ 输入预算(总预算 = num_ctx − system − schema − 输出预留),超限**继续细分**,绝不截断;
2. 逐块摘要(map)→ 归并(reduce);**摘要之外必须产出 facts 表与 coverage**(未覆盖的 block 不得静默丢弃);
...
**职责:** 生成逐场景口播稿,**每个场景引用源 block/page**(P0-02)。
...
- 数字/日期/专名与 facts 表**确定性一致性检查**(字符级比对),不符 → 修复或 needs_review;
...
**职责:** 为每场景选择视觉来源并生成图像提示词。**写 scene_plan.json,不覆写 script.json。**

```json
{
  "schema_version": "1.0",
  "style": { "name": "flat-illustration", "prefix": "…", "negative": "text, watermark, words, letters" },
  "scenes": [
    { "id": "sc01",
      "visual_desc": "演播台上一块大屏幕,屏幕上是报告封面",
      "visual_source": "generated",
      "image_prompt": "扁平插画风格,……,无文字,无水印",
      "aspect": "16:9",
      "extracted_ref": null }
  ]
}
...
| 值 | 含义 | 使用条件 |
|----|------|----------|
| `extracted_image` | 复用文档内嵌图片 | 图片与场景语义匹配 |
...
**职责:** 并行生产音频/图像,写 `assets_manifest.json` + 逐场景 checkpoint。**不覆写 scene_plan.json。**

**4.6.1 音频(TTS):**

| Provider | 特点 | timing marks |
|----------|------|--------------|
| edge-tts(默认) | 免费、中文音色多;⚠️ 调微软在线服务,非离线 | ✅ 边界事件(WordBoundary/SentenceBoundary) |
| OpenAI TTS(可选) | 质量高、付费 | ✅ 原生 speech marks |
| CosyVoice2(可选) | 本地离线 | ✅(对齐器) |

**TTSProvider 契约(输出):** `{audio_path, duration_s, timing_marks: [{text, start_s, end_s}], provider_metadata}` — timing_marks 是 P6 字幕的主数据源。

- 并发 ≤ 4(信号量);失败重试见 7.3;**静音占位仅限 preview 模式,正式交付 TTS 失败 = failed/needs_review**;
- 实测时长写入 assets_manifest(供 P7 render_timeline 使用)。

**4.6.2 图像:**

| Provider | 特点 |
|----------|------|
| FAL(默认) | 精确 model_id 冻结于版本化配置(见 5.1 与 P0-09),preflight 校验 |
...
**接口:** `produce_assets(scene_plan, providers, cache, cfg) -> AssetsManifest`

### 4.7 P6 字幕生成 `p6_subtitles`

**职责:** 生成 Cue 级时间轴(先于合成,被 P7 烧录)。**以 TTS timing marks 为主,不以 ASR 回读为主。**

**三级策略(依序,后一级仅为兜底):**

1. **原生边界(主):** 消费 TTSProvider 的 timing_marks(edge-tts 边界事件 / speech marks)直接生成 Cue;
2. **强制对齐(次):** 无原生 marks 的 Provider,用针对既定文本的 forced aligner(如 whisperX/对齐器),把原文对齐到音频;
3. **ASR 回读(兜底+QC):** faster-whisper(small,本地)回读音频做**文本覆盖检查**(发现漏读/错读),时间戳仅在无前两级能力时以"低置信度兜底"使用——**ASR 词时间戳不等于强制对齐,不得单独支撑 <300ms 硬承诺**。

**字幕可用性(P2-04):** CJK 字体打包入库、每行 ≤ 18 字、最多两行、最短显示时长 0.8s、按标点断句、中英混排/数字专名发音词典、对比度与安全区(横/竖屏分别定义)。**Scene ≠ 一条字幕:一个场景通常多条 Cue。**

**接口:** `make_subtitles(scene_plan, assets, cfg) -> Subtitles`

### 4.8 P7 视频合成 `p7_compose`

**职责:** 素材 → staging 成片。**基于 ffmpeg 8.x 参数数组调用(本机实测 8.1.2),不锁死旧 6.x,以 doctor 功能探测为准。**

**render_timeline 单一事实源(P1-04):** P5 实测时长回填后,Python 侧计算 `{lead_s, narration_offset_s, trail_s, scene_duration_s, fade_out_start_s, bgm_fade_start_s, total_s}`,P6 字幕、P7 合成、P8 时长检查共用同一 timeline,**所有 ffmpeg 参数为纯数值**(fade/afade 的 st 不接受算式字符串,本机已复现报错)。

**合成流程:**

1. **每场景片段:** 图像按目标画布**等比 scale + crop/pad**(16:9 与 9:16 参数化,禁止无条件拉伸)→ zoompan Ken Burns(方向按场景 id 哈希轮换)→ 淡入淡出(数值参数)→ 与旁白音频合成(lead/trail 留白);
2. **统一流参数(concat 前提):** 每片段强制 codec/profile/pix_fmt=yuv420p/分辨率/fps/timebase/48kHz 双声道一致,否则 concat demuxer 报流不匹配;
3. **拼接:** concat demuxer;
4. **背景音乐(可选):** 循环 + **旁白侧链 ducking(sidechaincompress)**,BGM 音量 -18dB 级;
5. **响度:** 先最终混音,再 loudnorm 两遍(或等价可复现流程),校验最终 LUFS/true peak;
6. **字幕烧录:** ASS + libass,`--no-burn-subs` 只跳过烧录(字幕文件照常产出);
7. **编码:** `-c:v libx264 -preset medium -crf 20 -profile:v high -pix_fmt yuv420p -c:a aac -b:a 128k -ar 48000 -movflags +faststart`;
8. 输出到 `staging/`,写 `render_manifest.json`;**final/ 由 P8 门禁通过后原子晋升。**

**子进程安全:** 参数数组调用、`shell=False`;路径先规范化并校验仍在 Job 根内;对空格/Unicode/引号/盘符/长路径建安全测试。

**接口:** `compose(scene_plan, assets, subtitles, cfg) -> RenderManifest`

### 4.9 P8 质检与发布 `p8_qc`

**职责:** 交付门禁。硬失败项不过关 = Job failed,staging 保留、不得晋升。

| 检查 | 方法 | 判定 |
|------|------|------|
| 可播放性 | ffprobe + **全量 decode-to-null** | 硬失败 |
| 编码规格 | H.264 High / yuv420p / 分辨率 / fps / AAC 48kHz 双声道 / faststart | 硬失败 |
...
| 事实覆盖 | script 的 source_block_ids vs grounded_summary 覆盖 + 未覆盖章节 | chapter_skipped / 证据不足 = **硬失败** |
| 占位率 | placeholder 计数 + 重复图 + 生成图乱字(可选 VLM) | 超上限 needs_review |

**状态分级:** succeeded / succeeded_with_warnings / needs_review / failed(映射 CLI 退出码,见 7.1)。**发布:** final 目录先写 staging 名,门禁通过后 os.replace 晋升。

**接口:** `run_qc(job, render_manifest, scene_plan, assets, subtitles) -> QCReport`
...
- 导出失败仅记录日志与质检报告,MP4 照常交付。

**接口:** `export_draft(scene_plan, assets, subtitles, cfg) -> Path | None`
...
| LLM | **Ollama qwen3:14b(本地默认)** / DeepSeek / Gemini / GPT | 统一 LLMProvider | **结构化输出优先**;token 预算实测;本地并行 1 起步(见 5.3) |
| TTS | **edge-tts** / OpenAI TTS / CosyVoice2 | edge-tts | 免费、音色好;**输出边界事件用于字幕**;注意在线服务与隐私边界(见 7.4) |
| 图像 | **FAL(精确端点冻结)** / ComfyUI 本地 | FAL | **model_id 待 M0 Spike 实测冻结,候选 `fal-ai/flux-2/klein/4b/base`(FAL 官方文档)或 Hermes 网关对应模型**;契约快照 + preflight(见 P0-09) |
| 视频合成 | **ffmpeg** / moviepy / Remotion | ffmpeg | 本机 8.1.2 full build(libx264/libass/zoompan 实测可用);**不锁死 6.x,固定已验证 build + 功能探测**;参数数组调用 |
| 字幕对齐 | **TTS marks / forced aligner** / faster-whisper | marks 为主,whisper 兜底 | 见 4.7;whisper small 本机 CUDA 路径 M0 基准 |
| 编排 | **asyncio + state.json** / n8n / Prefect | asyncio | 单机线性主链+少量 fan-out 足够;**先做对产物/状态/锁/request_id,再谈平台**(P2-06) |
| 契约 | **Pydantic → JSON Schema** | Pydantic 单一来源 | 生成 Draft 2020-12 Schema 入库,CI 防漂移(P2-01) |
...
| ffmpeg | **本机 8.1.2 full build(libx264/AAC/libass/zoompan 已确认)**;winget 不锁版本,以 doctor 功能探测为准 |
| GPU | RTX 5070,12227 MiB |
| Ollama | 0.32.15;qwen3:14b = 14.8B Q4_K_M ≈ **9.3GB**,最大 ctx 40960;**默认 ctx <24GB 显存是 4K** |
| 并发策略 | **OLLAMA_NUM_PARALLEL=1 起步**(当前环境变量均未设置);12GB 单卡 + 9.3GB 模型 + whisper 并存无基准,升 2 需先基准证明;阶段切换 keep_alive=0 卸载 |
| 上下文 | num_ctx 固定(建议 8192)并在 API 请求中显式传入;token 预算实测,不按字符估算 |
| 网络 | Ollama(localhost:11434)、edge-tts(微软在线)、FAL、可选云 LLM |
| 密钥 | FAL_KEY 经 .env(python-dotenv);云 LLM 可选;密钥不进 manifest/日志 |

**GPU 全局调度器:** Ollama / faster-whisper / 本地 ComfyUI 互斥或限额;OOM、CPU offload、503 过载 → 明确定义降级或失败,不静默降性能后仍宣称满足 SLO。
...
doc2video/
├── src/doc2video/
│   ├── cli.py                  # run/resume/batch/watch/report/doctor
...
│   ├── contracts/              # Pydantic 模型(单一契约来源)
│   ├── schemas/                # 由 contracts 生成的 JSON Schema(提交+CI 防漂移)
│   ├── state.py                # 状态机/指纹/原子提交/锁/事件日志
│   ├── cache.py                # 内容寻址缓存(canonical JSON + SHA-256)
│   ├── gpu_sched.py            # GPU 互斥/限额/keep_alive
│   ├── prompts/                # 全部 Prompt(版本化,附录 B)
│   └── qc/                     # 硬门禁检查项
...
├── workspace/                  # 运行时生成(勿提交)
│   └── <job_id>/
│       ├── manifest.json       # P0(来源/哈希/配置指纹/privacy_mode)
│       ├── state.json          # 状态机快照(见 6.3)
│       ├── events.jsonl        # 追加式审计日志(状态转移/attempt/请求)
│       ├── input/              # 原始文档不可变快照
│       ├── parsed.json         # P1
...
│       ├── script.json         # P3
│       ├── scene_plan.json     # P4
│       ├── assets_manifest.json   # P5(音频/图像/timing_marks/request_id/checkpoint)
│       ├── assets/{audio,images,bgm}/
│       ├── subtitles.json/.srt/.ass   # P6
│       ├── render_manifest.json   # P7(timeline/命令参数/产物哈希)
│       ├── staging/            # 未过门禁的成片
│       ├── final/output.mp4    # 仅 P8 门禁通过后存在
│       ├── drafts/             # 剪映草稿(可选,P9)
│       └── qc_report.json      # P8
└── README.md
```
...
> 所有阶段产物均为**真正的 Draft 2020-12 Schema**(由 Pydantic 模型生成)。以下为 scene_plan 的 Schema 本体(不是数据实例):
    ...
    "scenes": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "narration": { "type": "string", "minLength": 20, "maxLength": 500 },
          "est_duration_s": { "type": "number", "minimum": 5, "maximum": 60 },
...
**语义校验(跨文件,Schema 之外):** ID 唯一;引用集合完全匹配(source_block_ids ⊆ parsed.blocks);路径受限于 Job 根;文件存在;内容哈希与媒体探测通过。

### 6.3 状态机与断点续跑(P0-04 重写)

**状态枚举(每阶段):**

```
pending → running → committing → succeeded | succeeded_with_warnings | needs_review | failed | cancelled
succeeded → invalidated → pending   (上游变更/配置变更/人工修改触发级联失效)
```

- 顶层不存可漂移的 `phase`,由各阶段状态推导;上游未 succeeded 时下游不得运行;
- **stage_fingerprint:** 有序输入哈希 + 本阶段有效配置 + Prompt 哈希 + Provider/模型及修订 + Schema 版本 + pipeline/code 版本 + 工具版本;**任一变 → invalidated;**
- **原子提交协议:** 所有 JSON/媒体先写同文件系统唯一临时文件 → flush → fsync → close → os.replace;多文件阶段以最后提交的 artifact_manifest 为唯一可见提交点,消费者忽略未被 manifest 引用的文件;
...
- **锁:** 每 Job 跨进程排他锁 + revision(防双 resume);每个全局缓存键独立锁;P5 各任务写独立结果,单写者确定性汇总;
- **checkpoint:** P5 按 scene_id × asset_kind 保存逐项状态;中断只重做未完成项;
- **events.jsonl:** 追加式记录状态转移/attempt/失效原因/人工覆盖/Provider 请求(P2-02),用于审计与快照损坏重建;
...
doc2video run <input> [options]        # 全流程
    ...
    --privacy offline|approved_cloud|unrestricted
    --llm local | cloud                # 受 privacy_mode 约束
doc2video resume <job_id>              # 断点续跑(不重提远端请求)
doc2video batch <dir> [--watch] [--jobs 1]
doc2video report <job_id>              # 质检摘要
doc2video doctor                       # 环境预检(ffmpeg 滤镜/字体/Ollama/网络/磁盘)
```
...
- watch:轮询 10s;**不修改源文件**;等待 size/mtime 稳定且可打开 → 复制到 Job staging → 验证 magic+哈希 → 原子入队;
- 同 Job 互斥:排他锁防重入。
...
| terminal | 401/403、非法参数、Schema 不兼容、审核拒绝 | 立即失败,不重试 |
| auth / quota | 密钥失效、额度耗尽 | 失败 + 明确报告;云端降级受 privacy_mode 约束 |
...
- **承认"服务端已接受、客户端未存 request_id"的未知提交窗口**:无幂等/按 UUID 查询能力时语义为 at-least-once → 定义 `unknown_submit` 状态、远端核对、重复结果检测、人工/自动处置、单 Job/单日费用上限、孤儿请求清理;
- 服务端自动重试计入 attempt 与成本审计。

**重试预算:** 单调用 ≤ 3 次、单场景 ≤ 5 次、整 Job deadline 可配;熔断后 failed。

### 7.4 隐私模式与数据外发边界(P0-08)

| 模式 | 允许的外发 | 说明 |
|------|-----------|------|
| `offline` | 无(纯本地:Ollama/whisper/本地 ComfyUI/本地 TTS) | 文档与旁白不出本机 |
| `approved_cloud`(默认) | 白名单 Provider(默认 FAL、edge-tts) | 云端 LLM 降级需事前授权 |
| `unrestricted` | 任意已配置 Provider | — |

- 每个 Provider 声明**接收字段清单、目的地、留存、用途、失败替代策略**;manifest 记录实际外发清单(不含密钥);
- FAL:敏感作业显式关闭不必要 I/O 存储(X-Fal-Store-IO),设置媒体过期与 ACL;其他云服务同等级条款审查;
- 日志/state/错误响应/Prompt **脱敏**(密钥/PII/正文);
- 涉及未公开、个人、受监管文档时,云端降级**事前授权**,否则 needs_review/failed;
...
方案保持纯 Python 独立运行。可选:封装为 Hermes skill;讲稿润色可用 Hermes 子代理;FAL 可复用 Hermes 网关或直连(以解耦优先)。
...
| 级别 | 范围 | 工期 |
|------|------|------|
| **PoC(快乐路径)** | M0–M4:有限输入、单 Provider、无 watch/OCR 强承诺 | 2–3 周 |
| **MVP(功能完整)** | M0–M7:四类输入、真实 Provider、断点续跑、量化验收 | 4–6 周 |
| **发布级(无人值守)** | MVP + 安全/隐私/故障注入/成本/运维硬化 + 真实 smoke 门禁 | 6–8 周 + 20–30% 缓冲 |
...
| 里程碑 | 内容 | 验收 |
|--------|------|------|
| **M0 契约+骨架+Spike(2–3 天)** | Pydantic 契约与 Schema 生成、state 原语(原子提交/锁/指纹)、CLI、doctor;**技术 Spike:FAL 精确端点实测冻结、本机 ffmpeg 滤镜/字体/CJK 验证、whisper CUDA 与 Ollama 并行基准** | 空管线跑通;Spike 结论落 config |
| **M1 解析器(2–3 天)** | P0/P1 完整(阅读顺序/OCR 逐页/资源限制)+ 测试矩阵 | 4 类输入矩阵全绿 |
| **M2 理解+讲稿(3–4 天)** | grounded summary(事实表/覆盖)、源引用讲稿、事实一致性检查、LLM 能力协商 | 每场景可追溯;事实检查测试通过 |
| **M3 分镜+素材(3–5 天)** | scene_plan、visual_source、assets_manifest、缓存、timing marks、成本上限、checkpoint | 素材齐备;缓存命中零调用 |
| **M4 字幕+合成(3–5 天)** | P6(marks 主路径)、P7(render_timeline/横竖屏/staging) | 成片可播放;横竖屏;字幕达标 |
| **M5 质检+发布(2–3 天)** | P8 硬门禁、原子晋升、batch/watch、P9 草稿导出 | 门禁拒绝伪成功;断点续跑正确 |
| **M6 安全+运维(2–3 天)** | 隐私模式、脱敏、配额/GC、故障注入矩阵、许可证清单 | 注入测试全过 |
...
# 3. Ollama(已装 0.32.15;显式并发=1 起步,上下文在请求中传)
#    OLLAMA_NUM_PARALLEL=1  (先不改,以基准为准)

# 4. 密钥:.env(FAL_KEY 等),由 python-dotenv 加载,勿提交

# 5. 环境预检
doc2video doctor   # ffmpeg 滤镜/编码器、CJK 字体、Ollama 模型/上下文、
...
- 提交 uv.lock、Prompt 版本、Schema、fixtures 授权记录。
...
| 层级 | 内容 | 工具 |
|------|------|------|
| 单元 | 解析器(层级/阅读顺序)、token 预算、事实检查、字幕 Cue 生成、缓存键、契约生成 | pytest |
| 契约 | Pydantic 模型 ↔ 生成 Schema 无漂移(CI)+ 跨文件语义校验 | pytest + CI |
| 集成 | 阶段串联,供应商 mock;ffmpeg 滤镜真实执行(短 fixture) | pytest-asyncio |
...
**验收必须证明:** 既有 request_id 不重提;unknown_submit 受控;截断产物不被接受;不产伪成功。

### 9.2 验收标准(量化,修订)

| # | 标准 | 指标 |
|---|------|------|
| AC-01 | 四类输入端到端产出可播放成片 | 4/4 矩阵通过(OCR 样例另计) |
| AC-02 | 字幕-语音同步(**测量口径修正**) | 人工真值集:句/词起止误差 **P95 < 300ms**,报告 P50/P95/max/覆盖率/离群比;若一期不建真值集,删除硬指标、降级为预览目标 |
| AC-03 | 成片规格 | ffprobe 验证:H.264 High / yuv420p / 1080p / 25fps / AAC 48kHz 双声道 / faststart |
| AC-04 | 讲稿保真 | 每场景 source_block_ids 非空且覆盖章节 100%;facts 表数字/专名一致性检查通过 |
| AC-05 | 断点续跑 | 注入 kill 后 resume:已完成阶段跳过、缓存命中、已有 request_id 不重提、unknown_submit 受控 |
| AC-06 | 稳定性 | 批处理 10 文件零未处理异常;门禁拒绝一切伪成功样例 |
| AC-07 | 性能 SLO(分位数) | 冷/热缓存 × 短文/5000 字 × 横/竖屏的 P50/P95 分别测量并达标(**云排队时间单独报告,不并入"端到端")** |
| AC-08 | 成本 | 同一 Job 二次运行云端调用 = 0(全局缓存);单 Job 费用 ≤ 上限 |

### 9.3 质检指标定义

静音:每场景语音能量覆盖 + 连续静音 > 2s 段落数;黑帧:blackdetect 时间连续性(按风格校准);字幕:原文覆盖率/交叠/越界/P95/max;成片:全量 decode-to-null + 规格校验。
...
| R3 | 图像生成失败/排队超时 | 中 | 中 | 重试 + 占位(上限)+ request_id 持久化 + 成本封顶 |
...
| R11 | 隐私/数据外发违规 | 中 | 高 | privacy_mode 三档 + 外发清单 + 脱敏 + 授权门禁(见 7.4) |
| R12 | 密钥泄露/日志泄密 | 低 | 高 | .env + 脱敏 + 最小权限 + 跨项目密钥隔离 |
| R13 | Provider 端点/Schema/计费漂移 | 中 | 高 | 精确端点 + Schema 快照 + preflight fail closed;fallback 显式配置 |
| R14 | 远端重复请求/双重重试计费 | 中 | 高 | request_id 持久化 + 幂等键 + unknown_submit 语义 + 封顶 + 孤儿清理 |
| R15 | 磁盘满/缓存损坏/状态截断 | 中 | 高 | 原子提交 + 配额 + 缓存命中校验(SHA/可解码)+ GC/保留期 |
| R16 | GPU OOM/CPU offload/批处理雪崩 | 中 | 中 | GPU 调度器 + NUM_PARALLEL=1 起步 + 基准后升级 + 明确失败而非静默降级 |
| R17 | **许可证/分发义务** | 中 | 高 | PyMuPDF AGPL/商业双许可;edge-tts LGPL-3.0;本机 ffmpeg 为 GPL build(含 libx264)。**分发闭源客户端/打包前必须完成法务复核**,否则发布阻断 |
| R18 | 输入恶意文档(PDF/DOCX 注入、ZIP bomb) | 低 | 高 | quarantine + 资源限额 + 路径 containment + shell=False |
| R19 | LLM 幻觉/敏感领域错误 | 中 | 高 | 事实一致性检查 + source 引用 + 高风险领域不承诺全自动、需人工复核 |
...
### 附录 A.端到端示例(500 字短文,节选)

**P3 产出 `script.json`(场景带源引用):**

```json
{ "scenes": [
  { "id": "sc01", "narration": "大家好,今天我们聊聊咖啡因和睡眠。", "est_duration_s": 5.1,
    "source_block_ids": ["b1"], "source_pages": [1] },
  { "id": "sc02", "narration": "咖啡因半衰期约五到六小时,下午的咖啡到晚上还剩一半。", "est_duration_s": 8.9,
    "source_block_ids": ["b3"], "source_pages": [1] }
] }
```

**P5 后 `assets_manifest.json` 回写:** 每场景 `audio{duration_s, timing_marks[]}`、`image{path, request_id, cache_key}`。

**P6 字幕 Cue(由 timing_marks 生成,一个场景多条):** `[{"id":"sc01_c1","text":"大家好,","start":0.6,"end":1.9}, …]`

**最终:** `final/output.mp4`(约 37s,目标区间内)+ subtitles.srt/ass + qc_report.json。

### 附录 B.关键 Prompt 模板(全部入库版本化)
...
3. 只使用【相关原文块】与【事实表】中的信息,不得编造;数字/日期/专名必须与事实表完全一致;
4. 输出 JSON:{"scenes":[{"id":"sc01","chapter":"…","narration":"…",
   "est_duration_s":…,"source_block_ids":[…],"source_pages":[…]} ]}
...
**B.3 摘要与事实抽取(P2,`prompts/summarizer.txt`):** 逐块输出 `{summary, facts[{kind,text,source_block_ids}], coverage}` 三字段 JSON,归并模板同构(全文略,版本化于仓库)。

### 附录 C.ffmpeg 命令集(纯数值参数,横竖屏参数化)

> 所有时间参数由 Python 按 render_timeline **预先算成纯数值**(如 `FADE_OUT_ST=12.4`),经参数数组传给 ffmpeg;**禁止在 st=/d= 中拼算式字符串**(本机 8.1.2 已复现 `Unable to parse st option value`)。

**C.1 单场景(Ken Burns + 旁白 + 淡入淡出,16:9):**

```bash
ffmpeg -y -loop 1 -i sc01.png -i sc01.mp3 -filter_complex \
"[0:v]scale=3840:2160:force_original_aspect_ratio=increase,crop=3840:2160,zoompan=z='min(zoom+0.0008,1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=625:s=1920x1080:fps=25,fade=t=in:st=0:d=0.3,fade=t=out:st=12.1:d=0.3,format=yuv420p[v];[1:a]adelay=500|500,apad[a]" \
-map "[v]" -map "[a]" -t 12.7 -c:v libx264 -preset medium -crf 20 -profile:v high \
-c:a aac -b:a 128k -ar 48000 -ac 2 scene01.mp4
```

> 竖屏 9:16:canvas 换为 `s=1080x1920`,scale/crop 目标同步更换;d= 与 -t 由 render_timeline 代入(lead 0.5s + 音频 11.7s + trail 0.5s)。

**C.2 concat 拼接:** `ffmpeg -y -f concat -safe 0 -i scenes.txt -c copy joined.mp4`(每片段流参数已强制统一,见 4.8)

**C.3 BGM ducking + 混音 + 响度:**

```bash
ffmpeg -y -i joined.mp4 -stream_loop -1 -i bgm.mp3 -filter_complex \
"[0:a][1:a]sidechaincompress=threshold=0.03:ratio=8:attack=20:release=400[bgm];\
[0:a][bgm]amix=inputs=2:duration=first:normalize=0,loudnorm=I=-16:TP=-1.5:LRA=11,aresample=48000[a]" \
-map 0:v -map "[a]" -c:v copy -c:a aac -b:a 128k mixed.mp4
```
...
-c:a aac -b:a 128k -ar 48000 -movflags +faststart staging/output.mp4
```
...
| 评审方 | 日期 | 对象 | 结论 | 修订 |
|--------|------|------|------|------|
| gpt-5.6-sol(技术审查) | 2026-08-27 | v0.1 | **不通过**:11 P0 / 12 P1 / 2 P2(报告:`review_report.md`) | 全部 P0/P1 意见已修订 → **v0.2**(阶段重编号 P0–P9、可追溯讲稿、真实 Schema、崩溃安全状态机、token 预算、ffmpeg 数值参数、失败关停、隐私模式、FAL 端点冻结、字幕 marks、发布 smoke 门禁、工期重估、风险扩充) |
| gpt-5.6-sol(复审) | 待执行 | v0.2 | 待填写 | — |

---

*本文档 + 仓库内 contracts/schemas/prompts/config/fixtures 共同构成开发依据;冲突时以仓库内版本化契约为准。修订须同步版本号与评审记录。*
